...
def extract_features_batch(model, config, source_path, target_path, voxel_size, device):
  ...
  for fo in folders:
    ...
    for i, fi in enumerate(files):
      ...
      if i % 100 == 0:
        rootLogger.info(f"{i} / {len(files)}: {save_fn}")
...
def do_registration(feature_path, voxel_size):
  ...
  for s in sets:
    ...
    rootLogger.info(f"Writing the trajectory to {feature_path}/{set_name}.log")
...
def do_feature_evaluation(source_path, feature_path, voxel_size, num_rand_keypoints=-1):
  ...
  tau_1 = 0.1  # 10cm
  tau_2 = 0.05  # 5% inlier
  rootLogger.info("%f %f" % (tau_1, tau_2))
  recall = []
  for s in sets:
    set_name = s[0]
    ...
    rootLogger.info(f'{set_name}: {mean_recall} +- {std_recall}')
  for r in recall:
    rootLogger.info("%s : %.4f" % (r[0], r[1]))
  scene_r = np.array([r[1] for r in recall])
  rootLogger.info("average : %.4f +- %.4f" % (scene_r.mean(), scene_r.std()))
